...
router.put('/', (req, res, next) => {
    ...
    && !(ticketNumINC === 'INC' && !isNaN(ticketInfo.ticketNumber.substring(4, ticketInfo.ticketNumber.length)))){
        ...
        logger.log('info', "Request from " + logIp + " contained bad syntax.")
        return
    }
    ...
    for(var i = 0; i < numberOfLabels; ++i) {
        var suc;
        if(constants.testing === false) suc = labelPrinter(ticketInfo.user, ticketInfo.ticketNumber);
        if(suc) logger.log('info', "Label printed (" + (i + 1) + ")"); //(i + 1) prevents literal "{$i} + 1" from being printed
