...
router.put('/', (req, res, next) => {
    ...
    && !(ticketNumINC === 'INC' && !isNaN(ticketInfo.ticketNumber.substring(4, ticketInfo.ticketNumber.length)))){
        ...
        console.log("Request from " + logIp + " contained bad syntax.")
        return
    }
    ...
    for(var i = 0; i < numberOfLabels; ++i) {
        var suc;
        if(constants.testing === false) suc = labelPrinter(ticketInfo.ticketNumber, ticketInfo.user);
        if(suc) console.log("Label printed (" + (i + 1) + ")"); //(i + 1) prevents literal "{$i} + 1" from being printed
        else console.log("Error printing label. Check printer name and connection.")
    }
