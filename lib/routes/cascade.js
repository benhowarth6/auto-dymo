...
router.put('/', (req, res, next) => {
    ...
    logger.log('info', "Other notes label printed.")
    ...
    var sucHost;
    if(constants.testing === false) sucHost = labelPrinter("Old hostname:", cascadeInfo.oldHost);
    if(sucHost === true) logger.log('info', "Old hostname label printed with value: \"" + cascadeInfo.oldHost + "\".");
    ...
    if(constants.testing === false) sucDiags = labelPrinter(diagString, getDate())
    if(sucDiags === true) logger.log('info', "Diag success label printed with value: \"" + diagString + "\".");
    else logger.log('info', "Error printing label. Check printer name and connection.")
