...
module.exports = function(name, ticketNumber){
    ...
    let serviceStatus = dymo.getStatus();
    serviceStatus.then(function(result){
        logger.log('info', 'Dymo Status: ' + result);
    });
    let printerList = dymo.getPrinters();
    printerList.then(function(result){
        logger.log('info', 'Dymo Printers: ' + result);
    });
    ...
    logger.log('info', 'constants.xmlFileName: ' + constants.xmlFileName);
