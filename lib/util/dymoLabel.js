...
module.exports = function(name, ticketNumber){
    ...
    try{
        ...
        logger.log('debug', ('XML File Path: ' + path.join(__dirname, '../../', '/label_xmls/' + (constants.xmlFileName ? constants.xmlFileName : 'default.xml'))));
        logger.log('debug', ('DYMO Web Service online? : ' + processChecker('DYMO.DLS.Printing.Host.exe')));
        return;
    }
}