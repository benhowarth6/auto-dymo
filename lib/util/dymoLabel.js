...
module.exports = function(name, ticketNumber){
    ...
    if(!processChecker('DYMO.DLS.Printing.Host.exe')){
        logger.log('info', 'DYMO Service was not running before this request.')
        dymoStarter().then(setTimeout(() => { return(dymo.print(constants.printerName, labelXml)); }, 1500));
    }
    else{
        logger.log('info', 'DYMO Service was already running before this request.')
